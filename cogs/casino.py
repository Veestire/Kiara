...
class Casino:
    ...
    async def classicroulette(self, ctx, amount:int, choice):
        ...
        spinemotes = [
        '<:r0:451727652219518986>', '<:r1:451726112414892032>','<:r2:451726112284868618>', '<:r3:451726112381206538>',
        '<:r4:451726111949193219>', '<:r5:451726112213696512>', '<:r6:451726112238600202>',
        '<:r7:451726112201113620>', '<:r8:451726112343588864>', '<:r9:451726111970295809>', '<:r10:451726112163102731>',
        '<:r11:451726112100450305>', '<:r12:451726112314097704>', '<:r13:451726112423149568>', '<:r14:451726112343719937>'
        ]
        pointer = ['<:line:474567346833588244>', '<:arrowup:451728056764334081>']
            ...
            if hitcolor != None:
                if hitcolor == choice.lower():
                    if choice.lower() == "green":
                        ...
                        await ctx.send(f"{pointer[0]} {pointer[1]} {pointer[0]}")
                        await ctx.send(f"It landed on {hitcolor}! You win {amount*15} gold!")
                        profile.coins += (amount*10)

                    else:
                        await ctx.send(f"{spinemotes[val1]} {spinemotes[hit]} {spinemotes[val3]}")
                        await ctx.send(f"{pointer[0]} {pointer[1]} {pointer[0]}")
                        await ctx.send(f"The roulette landed on {hit}! You bet on {hitcolor}, you win {amount*2} gold!")
                        profile.coins += (amount*2)

                else:
                    await ctx.send(f"{spinemotes[val1]} {spinemotes[hit]} {spinemotes[val3]}")
                    await ctx.send(f"{pointer[0]} {pointer[1]} {pointer[0]}")
                    await ctx.send(f"The roulette landed on {hit} which is {hitcolor}! Sorry, you lose {amount} gold.")
                    profile.coins -= amount
